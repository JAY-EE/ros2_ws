...
#define diff_m1_pwm 14
#define diff_m1_dir 27
...
void setup() {
  ...
  I2C_0.begin(21, 22);  // SDA, SCL pins for I2C_0
  I2C_1.begin(4, 16);   // SDA, SCL pins for I2C_1
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    gripper = incomingData.substring(commaIndex5 + 1).toInt();
    ...
    if (pitch == 1){
          analogWrite(diff_m1_pwm,180);
          digitalWrite(diff_m1_dir,HIGH);
          analogWrite(diff_m2_pwm,180);
    ...
    if (manual == 1){
      ...
      if ((target_1 == 0) && (target_2 == 0)) {
        ...
        if ((int)target_2 != (int)angle_2) {
          if (target_2 > angle_2)
            digitalWrite(motor2_dir, HIGH);  // Rotate motor 2 clockwise
          else
            digitalWrite(motor2_dir, LOW);   // Rotate motor 2 counterclockwise
