...
# Callback function for armClient data
def arm_callback(data):
    global rec_y, command, position, pitch, yaw, gripper
    rec_y = data.y
...
def arm_server():
    ...
    while rclpy.ok():
        ...
        global target, angles, isPreset, position, command,rec_y,pitch,yaw,gripper
        ...
        # Compute angles from input angles
        angle_rad = {
            'first': angles['first'] * pi / 180,
            'second': angles['second'] * pi / 180,
        }

        # Perform forward kinematics to get x, y, z
        ...
        # Update target angles based on predefined positions
        if position == 1:
            goToPos1()
        elif position == 2:
            goToPos2()
        ...
        # Send target angles to Arduino via serial communication
        values = [target['first'], target['second'],rec_y,pitch,yaw,gripper]
        if command == 99:
             values = [0,0,rec_y,pitch,yaw,gripper]
    ...
    # Shutdown ROS 2 node gracefully
